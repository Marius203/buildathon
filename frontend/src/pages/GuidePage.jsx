...
const LABEL_STYLE = { fontSize: "0.7rem", fontWeight: 900, letterSpacing: "0.14em", textTransform: "uppercase", color: "#888", marginBottom: 10 };
const CARD = { border: "2px solid var(--ec-black)", overflow: "hidden" };

const ACCOMMODATION = [
  {
    id: "camping", icon: "⛺", label: "Camping Standard",
    badge: "CEL MAI POPULAR", badgeColor: "#15803d",
    pros: ["Cel mai ieftin", "Comunitate mare", "Atmosferă autentică EC"],
    cons: ["Aduci propriul cort", "Duș la comun", "Zgomot noaptea"],
    tip: "Ajunge joi dimineață pentru loc bun lângă intrare.",
    price: "~100–150 RON / persoană",
    link: "https://electriccastle.ro",
  },
  {
    id: "glamping", icon: "🛖", label: "Glamping",
    badge: "SE TERMINĂ RAPID", badgeColor: "#dc2626",
    pros: ["Pat real + lenjerie", "Mult mai confortabil", "Fără cort propriu"],
    cons: ["Mult mai scump", "Se rezervă în ore de la lansare", "Disponibilitate limitată"],
    tip: "Rezervă în prima oră după lansarea biletelor.",
    price: "~600–1.200 RON / persoană",
    link: "https://electriccastle.ro",
  },
  {
    id: "hotel", icon: "🏨", label: "Hotel Cluj-Napoca",
    badge: "CONFORT MAXIM", badgeColor: "#1d4ed8",
    pros: ["Pat, duș, AC", "Orașul la îndemână", "Fără noroi garantat"],
    cons: ["30km distanță", "Naveta zilnică cu shuttle", "Pierzi atmosfera nopții"],
    tip: "Rezervă camera devreme — prețurile cresc cu 3x în zilele festivalului.",
    price: "~200–600 RON / noapte",
    link: "https://www.booking.com/city/ro/cluj-napoca.html",
  },
];
    ...
    <div style={CARD}>
      ...
      {point.hasTimetable ? (
        ...
        <div style={{ padding: "16px 20px" }}>
          ...
          <div style={{ fontSize: "0.85rem", color: "#444", marginBottom: 12, lineHeight: 1.6 }}>Pleacă continuu pe toată durata festivalului. Nu necesită rezervare — cumperi biletul online sau la stație.</div>
...
function AccommodationBlock() {
  ...
  return (
    <div style={CARD}>
      <div style={{ display: "flex", borderBottom: "2px solid var(--ec-black)" }}>
        {ACCOMMODATION.map(a => (
          <button key={a.id} onClick={() => setActive(a.id)} style={{ flex: 1, padding: "14px 6px", cursor: "pointer", border: "none", borderRight: "1px solid #e5e7eb", background: active === a.id ? "var(--ec-black)" : "var(--ec-white)", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, transition: "background 0.15s" }}>
            <span style={{ fontSize: "1.4rem" }}>{a.icon}</span>
            <span style={{ fontFamily: "var(--ec-font-display)", fontSize: "0.72rem", fontWeight: 800, color: active === a.id ? "var(--ec-white)" : "var(--ec-black)", textAlign: "center", letterSpacing: "0.02em" }}>{a.label}</span>
          </button>
        ))}
      </div>
      <div style={{ padding: "16px 20px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
          <div style={{ fontFamily: "var(--ec-font-display)", fontSize: "1.1rem", fontWeight: 800 }}>{opt.icon} {opt.label}</div>
          <span style={{ background: opt.badgeColor, color: "#fff", fontSize: "0.6rem", fontWeight: 900, letterSpacing: "0.1em", padding: "3px 8px" }}>{opt.badge}</span>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}>
          <div style={{ background: "#f0fff4", border: "1px solid #bbf7d0", padding: "10px 12px" }}>
            <div style={{ fontSize: "0.65rem", fontWeight: 900, letterSpacing: "0.1em", color: "#15803d", marginBottom: 6 }}>✓ AVANTAJE</div>
            {opt.pros.map((p, i) => <div key={i} style={{ fontSize: "0.8rem", color: "#166534", marginBottom: 3 }}>+ {p}</div>)}
          </div>
          <div style={{ background: "#fff8f8", border: "1px solid #fecaca", padding: "10px 12px" }}>
            <div style={{ fontSize: "0.65rem", fontWeight: 900, letterSpacing: "0.1em", color: "#dc2626", marginBottom: 6 }}>✗ DEZAVANTAJE</div>
            {opt.cons.map((c, i) => <div key={i} style={{ fontSize: "0.8rem", color: "#991b1b", marginBottom: 3 }}>− {c}</div>)}
          </div>
        </div>
        <div style={{ background: "#fff8e1", borderLeft: "3px solid #f59e0b", padding: "8px 12px", fontSize: "0.82rem", color: "#92400e", fontWeight: 600, marginBottom: 12 }}>⚡ {opt.tip}</div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontFamily: "var(--ec-font-display)", fontSize: "0.88rem", fontWeight: 800 }}>💰 {opt.price}</span>
          <a href={opt.link} target="_blank" rel="noreferrer" style={{ background: "var(--ec-black)", color: "var(--ec-white)", padding: "8px 16px", fontSize: "0.75rem", fontWeight: 800, fontFamily: "var(--ec-font-display)", letterSpacing: "0.06em", textDecoration: "none" }}>REZERVĂ →</a>
        </div>
      </div>
    </div>
  );
...
function PackingBlock() {
  const [checked, setChecked] = useState(() => {
    try { return JSON.parse(localStorage.getItem("ec_packing") || "{}"); } catch { return {}; }
  });
  ...
  function toggle(id) {
    ...
    setChecked(next);
    try { localStorage.setItem("ec_packing", JSON.stringify(next)); } catch {}
  }

  const filtered = filter === "all" ? PACKING_LIST : PACKING_LIST.filter(i => i.cat === filter);
  const total = PACKING_LIST.length;
  const done = PACKING_LIST.filter(i => checked[i.id]).length;
  const pct = Math.round((done / total) * 100);

  const catLabel = { must: "OBLIGATORIU", rec: "RECOMANDAT", opt: "OPȚIONAL" };
  ...
  return (
    <div style={CARD}>
      <div style={{ padding: "14px 20px", borderBottom: "1px solid #eee", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ fontFamily: "var(--ec-font-display)", fontWeight: 800, fontSize: "0.95rem" }}>🎒 Packed {done}/{total}</div>
        <div style={{ display: "flex", gap: 4 }}>
          {["all", "must", "rec", "opt"].map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{ padding: "4px 10px", fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", cursor: "pointer", background: filter === f ? "var(--ec-black)" : "none", border: `1px solid ${filter === f ? "var(--ec-black)" : "#e5e7eb"}`, color: filter === f ? "var(--ec-white)" : "#666" }}>
              {f === "all" ? "TOATE" : f === "must" ? "MUST" : f === "rec" ? "REC" : "OPT"}
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: "8px 20px" }}>
        {filtered.map(item => (
          <button key={item.id} onClick={() => toggle(item.id)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "10px 0", background: "none", border: "none", borderBottom: "1px solid #f5f5f5", cursor: "pointer", textAlign: "left" }}>
            <div style={{ width: 22, height: 22, border: `2px solid ${checked[item.id] ? "var(--ec-black)" : "#e5e7eb"}`, background: checked[item.id] ? "var(--ec-black)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.15s" }}>
              {checked[item.id] && <span style={{ color: "var(--ec-white)", fontSize: "0.7rem", fontWeight: 900 }}>✓</span>}
            </div>
            <span style={{ fontSize: "1rem" }}>{item.icon}</span>
            <span style={{ flex: 1, fontSize: "0.88rem", fontWeight: 600, color: checked[item.id] ? "#aaa" : "var(--ec-black)", textDecoration: checked[item.id] ? "line-through" : "none", transition: "all 0.15s" }}>{item.label}</span>
            <span style={{ fontSize: "0.6rem", fontWeight: 900, letterSpacing: "0.08em", color: catColor[item.cat], flexShrink: 0 }}>{catLabel[item.cat]}</span>
          </button>
        ))}
        {done === total && (
          <div style={{ textAlign: "center", padding: "16px 0 8px", fontFamily: "var(--ec-font-display)", fontWeight: 800, color: "#15803d", fontSize: "0.95rem" }}>✅ Ești gata pentru EC! 🏰</div>
        )}
      </div>
    </div>
  );
...
function CashlessBlock() {
  return (
    <div style={CARD}>
      <div style={{ padding: "16px 20px", borderBottom: "1px solid #eee" }}>
        <div style={{ fontFamily: "var(--ec-font-display)", fontWeight: 800, fontSize: "1rem" }}>💳 Cum funcționează cashless în 5 pași</div>
        <div style={{ fontSize: "0.78rem", color: "#666", marginTop: 4 }}>Electric Castle e 100% fără numerar. Orice — de la bere la shaorma — se plătește cu brățara.</div>
      </div>
      <div style={{ padding: "12px 20px" }}>
        {CASHLESS_STEPS.map((s, i) => (
          <div key={i} style={{ display: "flex", gap: 14, padding: "10px 0", borderBottom: i < CASHLESS_STEPS.length - 1 ? "1px solid #f5f5f5" : "none", alignItems: "flex-start" }}>
            <div style={{ width: 36, height: 36, background: "var(--ec-black)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <span style={{ fontFamily: "var(--ec-font-display)", fontWeight: 900, fontSize: "0.72rem", color: "var(--ec-white)", letterSpacing: "0.04em" }}>{s.step}</span>
            </div>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
                <span style={{ fontSize: "1rem" }}>{s.icon}</span>
                <span style={{ fontFamily: "var(--ec-font-display)", fontWeight: 800, fontSize: "0.88rem" }}>{s.title}</span>
              </div>
              <div style={{ fontSize: "0.8rem", color: "#555", lineHeight: 1.5 }}>{s.desc}</div>
            </div>
          </div>
        ))}
      </div>
      <div style={{ padding: "12px 20px", background: "#fff8e1", borderTop: "1px solid #fcd34d" }}>
        <span style={{ fontSize: "0.82rem", fontWeight: 600, color: "#92400e" }}>⚡ Sfat: Încarcă mai mult decât crezi. Mai ușor să ceri rambursare decât să stai la coadă la 2 noaptea.</span>
      </div>
    </div>
  );
}
...
function FoodBlock() {
  const [filter, setFilter] = useState("all");
  const types = ["all", "vegan", "breakfast", "comfort"];
  ...
  return (
    <div style={CARD}>
      <div style={{ padding: "14px 20px", borderBottom: "1px solid #eee", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
        <div style={{ fontFamily: "var(--ec-font-display)", fontWeight: 800, fontSize: "0.95rem" }}>🍕 Food trucks recomandate</div>
        <div style={{ display: "flex", gap: 4 }}>
          {[["all","TOATE"],["vegan","🌱 VEGAN"],["breakfast","🍳 DIMINEAȚĂ"],["comfort","🔥 COMFORT"]].map(([k, l]) => (
            <button key={k} onClick={() => setFilter(k)} style={{ padding: "4px 8px", fontSize: "0.62rem", fontWeight: 700, letterSpacing: "0.04em", cursor: "pointer", background: filter === k ? "var(--ec-black)" : "none", border: `1px solid ${filter === k ? "var(--ec-black)" : "#e5e7eb"}`, color: filter === k ? "var(--ec-white)" : "#666", whiteSpace: "nowrap" }}>{l}</button>
          ))}
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0 }}>
        {filtered.map((f, i) => (
          <div key={i} style={{ padding: "14px 16px", borderBottom: "1px solid #f0f0f0", borderRight: i % 2 === 0 ? "1px solid #f0f0f0" : "none" }}>
            <div style={{ fontFamily: "var(--ec-font-display)", fontWeight: 800, fontSize: "0.9rem", marginBottom: 3 }}>{f.name}</div>
            <div style={{ fontSize: "0.68rem", color: "#888", fontWeight: 700, letterSpacing: "0.04em", marginBottom: 5 }}>{f.type}</div>
            <div style={{ fontSize: "0.78rem", color: "#444", lineHeight: 1.4, marginBottom: 6 }}>{f.desc}</div>
            <div style={{ fontSize: "0.65rem", background: "#f0f0f0", display: "inline-block", padding: "2px 8px", color: "#555", fontWeight: 600 }}>📍 {f.tag}</div>
          </div>
        ))}
      </div>
      <div style={{ padding: "12px 20px", borderTop: "1px solid #eee", fontSize: "0.78rem", color: "#666" }}>
        + Zeci de alte food trucks. Aproape toți au cel puțin o opțiune vegană.
      </div>
    </div>
  );
}
...
function SafetyBlock() {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
      <div style={{ ...CARD, padding: "20px", gridColumn: "1 / -1", background: "var(--ec-black)", color: "var(--ec-white)" }}>
        <div style={{ fontSize: "0.65rem", fontWeight: 900, letterSpacing: "0.14em", color: "#888", marginBottom: 8 }}>LINIE DE URGENȚĂ</div>
        <div style={{ fontFamily: "var(--ec-font-display)", fontSize: "1.6rem", fontWeight: 900, letterSpacing: "0.02em", marginBottom: 4 }}>+40 741 069 443</div>
        <div style={{ fontSize: "0.82rem", color: "#aaa" }}>Safety Line EC — disponibil non-stop în timpul festivalului</div>
      </div>
      <div style={{ ...CARD, padding: "16px" }}>
        <div style={{ fontSize: "1.5rem", marginBottom: 8 }}>🦺</div>
        <div style={{ fontFamily: "var(--ec-font-display)", fontWeight: 800, fontSize: "0.88rem", marginBottom: 6 }}>Red Team</div>
        <div style={{ fontSize: "0.78rem", color: "#555", lineHeight: 1.5 }}>Găsești echipa Red Team în tot festivalul. Recunoști după vesta roșie. Caută-i oricând ai nevoie.</div>
        <div style={{ marginTop: 10, fontSize: "0.78rem", color: "#555", lineHeight: 1.5, borderTop: "1px solid #eee", paddingTop: 10 }}>🍹 <strong>Angel Shot</strong> — comandă la orice bar dacă te simți nesigur. Cineva din echipă va veni discret.</div>
      </div>

      <div style={{ ...CARD, padding: "16px", gridColumn: "1 / -1" }}>
        <div style={{ fontSize: "1.5rem", marginBottom: 8 }}>🏥</div>
        <div style={{ fontFamily: "var(--ec-font-display)", fontWeight: 800, fontSize: "0.88rem", marginBottom: 6 }}>First Aid</div>
        <div style={{ fontSize: "0.78rem", color: "#555", lineHeight: 1.5 }}>Posturi medicale lângă Main Stage, lângă Ping Pong Stage, în potcoava castelului și în camping lângă dușuri. Nu ești niciodată departe de ajutor.</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={CARD}>
      <div style={{ padding: "14px 20px", borderBottom: "1px solid #eee" }}>
        ...
        <div style={{ fontSize: "0.75rem", color: "#888", marginTop: 3 }}>Click pe o scenă pentru detalii</div>
      </div>

      <div style={{ position: "relative", background: "#f9f6f0", borderBottom: "1px solid #eee" }}>
        <svg viewBox="0 0 500 320" style={{ width: "100%", display: "block" }}>
          ...
          {STAGES.map(s => (
            <g key={s.id} onClick={() => setActive(active === s.id ? null : s.id)} style={{ cursor: "pointer" }}>
              ...
              <circle cx={s.x} cy={s.y} r={s.r}
                fill={active === s.id ? s.color : "var(--ec-white)"}
                stroke={s.color}
                strokeWidth={active === s.id ? 0 : 2.5}
                style={{ transition: "all 0.2s" }}
              />
              {active === s.id && (
                <circle cx={s.x} cy={s.y} r={s.r + 4} fill="none" stroke={s.color} strokeWidth="1.5" opacity="0.4"/>
              )}
              <text x={s.x} y={s.y - s.r - 5} textAnchor="middle" fontSize="8.5" fill={s.color} fontFamily="sans-serif" fontWeight="bold">
                {s.label.split(" ")[0]}
              </text>
            </g>
          ))}
          ...
          {/* Distance labels */}
          <text x="225" y="210" textAnchor="middle" fontSize="8" fill="#bbb" fontFamily="sans-serif">~10 min</text>
          <text x="345" y="175" textAnchor="middle" fontSize="8" fill="#bbb" fontFamily="sans-serif">~5 min</text>
        </svg>
      </div>

      {/* Stage detail panel */}
      {stage ? (
        <div style={{ padding: "14px 20px", borderTop: "1px solid #eee", display: "flex", gap: 12, alignItems: "flex-start" }}>
          <div style={{ width: 40, height: 40, background: stage.color, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <span style={{ color: "white", fontSize: "1.1rem" }}>🎵</span>
          </div>
          <div>
            <div style={{ fontFamily: "var(--ec-font-display)", fontWeight: 800, fontSize: "0.95rem", color: stage.color }}>{stage.label}</div>
            <div style={{ fontSize: "0.78rem", color: "#555", marginTop: 2 }}>{stage.sub}</div>
          </div>
        </div>
      ) : (
        <div style={{ padding: "12px 20px", fontSize: "0.75rem", color: "#aaa", textAlign: "center" }}>
          Selectează o scenă de pe hartă
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={CARD}>
      <div style={{ padding: "14px 20px", borderBottom: "2px solid var(--ec-black)", background: "var(--ec-black)" }}>
        <div style={{ fontFamily: "var(--ec-font-display)", fontWeight: 900, fontSize: "1rem", color: "var(--ec-white)", letterSpacing: "0.02em" }}>⚡ SFATURI DE LA CEI CU EXPERIENȚĂ</div>
        <div style={{ fontSize: "0.72rem", color: "#888", marginTop: 3 }}>Lucruri pe care le înveți doar după primul EC</div>
      </div>
      <div>
        {TIPS.map((t, i) => (
          <button key={i} onClick={() => setExpanded(expanded === i ? null : i)}
            style={{ width: "100%", display: "flex", alignItems: "flex-start", gap: 14, padding: "14px 20px", background: expanded === i ? "#fafafa" : "var(--ec-white)", border: "none", borderBottom: i < TIPS.length - 1 ? "1px solid #f0f0f0" : "none", cursor: "pointer", textAlign: "left", transition: "background 0.15s" }}>
            <span style={{ fontSize: "1.3rem", flexShrink: 0, marginTop: 1 }}>{t.icon}</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: "var(--ec-font-display)", fontWeight: 800, fontSize: "0.88rem", color: "var(--ec-black)", marginBottom: expanded === i ? 6 : 0 }}>{t.tip}</div>
              {expanded === i && <div style={{ fontSize: "0.8rem", color: "#555", lineHeight: 1.6 }}>{t.desc}</div>}
            </div>
            <span style={{ fontSize: "0.8rem", color: "#ccc", flexShrink: 0, marginTop: 2 }}>{expanded === i ? "▲" : "▼"}</span>
          </button>
        ))}
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <div className="ec-page-content">
        ...
        <div style={LABEL_STYLE}>VREME LA FESTIVAL</div>
        <WeatherBlock />

        <div style={{ ...LABEL_STYLE, marginTop: 32 }}>TRANSPORT DIN CLUJ-NAPOCA</div>
        <TransportBlock />
        <a href="https://www.google.com/maps/dir/Gara+Cluj-Napoca/Iulius+Mall+Cluj/Expo+Transilvania+Cluj/Banffy+Castle+Bontida" target="_blank" rel="noreferrer"
          style={{ display: "inline-block", marginTop: 10, fontSize: "0.82rem", fontWeight: 700, color: "var(--ec-black)", textDecoration: "underline", textUnderlineOffset: 3 }}>
          🗺️ Vezi ruta completă pe Google Maps →
        </a>

        <div style={{ ...LABEL_STYLE, marginTop: 40 }}>CAZARE</div>
        <AccommodationBlock />

        <div style={{ ...LABEL_STYLE, marginTop: 32 }}>CHECKLIST BAGAJ</div>
        <PackingBlock />

        <div style={{ ...LABEL_STYLE, marginTop: 32 }}>SISTEM CASHLESS</div>
        <CashlessBlock />

        <div style={{ ...LABEL_STYLE, marginTop: 32 }}>MÂNCARE & FOOD TRUCKS</div>
        <FoodBlock />
        ...
        <div style={{ ...LABEL_STYLE, marginTop: 32 }}>SFATURI DE LA VETERANI</div>
        <VeteranTipsBlock />
